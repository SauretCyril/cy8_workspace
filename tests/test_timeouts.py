...
def test_timeouts():
    ...
    if main_timeout_match:
        ...
        print(
            f"✅ Timeout workflow principal: {main_timeout} secondes ({main_timeout/60:.1f} minutes)"
        )
        if main_timeout == 600:
    ...
    if ws_timeout_match:
        ...
        print(
            f"✅ Timeout WebSocket: {ws_timeout} secondes ({ws_timeout/60:.1f} minutes)"
        )
        if ws_timeout == 600.0:
            print("   ✓ Configuré correctement à 10 minutes")
        else:
    ...
    print(
        "- Cela permet d'attendre des workflows très longs sans interruption prématurée"
    )
